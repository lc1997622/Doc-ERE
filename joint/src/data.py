    ...
    def __init__(self, data, ignore_nonetype=False):
        ...
        if 'PRECONDITION' in self.causal_relations:
            for re in self.causal_relations['PRECONDITION']:
                ...
                self.relations['PRECONDITION'].append('The temporal relation between ' + first + ' and ' + second + ' is precondition')
...
class myDataset(Dataset):
    ...
    def tokenize(self):
        ...
        for example in tqdm(self.examples, desc="tokenizing"):
            ...
            tokenized = {"input_ids": input_ids, "attention_mask": None, "event_spans": event_spans, "coref_labels": example.coref_labels, "temporal_labels": example.temporal_labels, "causal_labels": example.causal_labels, "subevent_labels": example.subevent_labels, "events_idx": example.events_idx, "doc_id": example.id,"pre_rlabel":example.relations_labels['PRECONDITION']}
...
class myNoTensorDataset(Dataset):
    def __init__(self, tokenizer, data_dir, split, max_length=512, ignore_nonetype=False, sample_rate=None):
        if sample_rate and split != "train":
            print("sampling test or dev, is it intended?")
        self.tokenizer = tokenizer
    ...
    def tokenize(self):
        ...
        for example in tqdm(self.examples, desc="tokenizing"):
            ...
            example.relations_labels['PRECONDITION'].extend([2])
            while len(example.relations_labels['PRECONDITION']) < self.max_length*2:
                example.relations_labels['PRECONDITION'].extend([self.tokenizer.pad_token_id])
    ...
    def to_tensor(self):
        for item in self.tokenized_samples:
            ...
            item["causal_labels"] = torch.LongTensor(item["causal_labels"])
            item["subevent_labels"] = torch.LongTensor(item["subevent_labels"])
            item["pre_rlabel"] =torch.FloatTensor(item["pre_rlabel"])
...
def collator(data):
    collate_data = {'pre_rlabel':[],"input_ids": [], "attention_mask": [], "event_spans": [], "splits": [0], "coref_labels": [], "temporal_labels": [],"causal_labels": [], "subevent_labels": [], "events_idx": [], "doc_id": []}
    for d in data:
        for k in d:
    ...
    collate_data["pre_rlabel"] = torch.stack(collate_data["pre_rlabel"])
...
def get_dataloader(tokenizer, split, data_dir="/home/lichao/ERE/MAVEN-ERE/data/MAVEN_ERE", max_length=128, batch_size=8, shuffle=True, ignore_nonetype=False, sample_rate=None):
...
if __name__ == "__main__":
    ...
    dd = tokenizer('<pad>l love you<pad>')
    aad = tokenizer.decode(dd['input_ids'])
